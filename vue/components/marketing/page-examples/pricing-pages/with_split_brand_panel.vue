...
<template>
  <div class="bg-white">
    <div class="bg-gray-50">
          ...
          <div class="flex items-center justify-between border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
            <div class="flex justify-start lg:w-0 lg:flex-1">
              <a href="#">
                ...
                <img class="h-8 w-auto sm:h-10" src="https://tailwindui.com/img/logos/mark.svg?color=cyan&shade=600" alt="" />
              </a>
            </div>
            <div class="-my-2 -mr-2 md:hidden">
              <PopoverButton class="inline-flex items-center justify-center rounded-md bg-gray-50 p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-cyan-500">
            ...
            <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
              <Popover class="relative" v-slot="{ open }">
                <PopoverButton :class="[open ? 'text-gray-900' : 'text-gray-500', 'group inline-flex items-center rounded-md bg-gray-50 text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2']">
                  <span>Solutions</span>
                    ...
                    <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                      <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                        <a v-for="item in solutions" :key="item.name" :href="item.href" class="-m-3 flex items-start rounded-lg p-3 hover:bg-gray-50">
                          <component :is="item.icon" class="h-6 w-6 flex-shrink-0 text-cyan-600" aria-hidden="true" />
                          <div class="ml-4">
              ...
              <Popover class="relative" v-slot="{ open }">
                ...
                <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
                  <PopoverPanel class="absolute left-1/2 z-10 mt-3 w-screen max-w-md -translate-x-1/2 transform px-2 sm:px-0">
                    <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                      <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                        <a v-for="item in resources" :key="item.name" href="#" class="-m-3 flex items-start rounded-lg p-3 hover:bg-gray-50">
                          <component :is="item.icon" class="h-6 w-6 flex-shrink-0 text-cyan-600" aria-hidden="true" />
                          <div class="ml-4">
                            <p class="text-base font-medium text-gray-900">{{ item.name }}</p>
                            <p class="mt-1 text-sm text-gray-500">{{ item.description }}</p>
                        ...
                        <div>
                          ...
                          <ul role="list" class="mt-4 space-y-4">
                            <li v-for="post in recentPosts" :key="post.id" class="truncate text-base">
                              <a :href="post.href" class="font-medium text-gray-900 hover:text-gray-800">{{ post.name }}</a>
                            </li>
                          </ul>
                        </div>
            ...
            <div class="hidden items-center justify-end space-x-8 md:flex md:flex-1 lg:w-0">
              ...
              <a href="#" class="inline-flex items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-cyan-600 py-2 px-4 text-base font-medium text-white shadow-sm hover:bg-cyan-700">Sign up</a>
            </div>
          </div>
        </div>
          ...
          <PopoverPanel focus class="absolute inset-x-0 top-0 z-10 origin-top-right transform p-2 transition md:hidden">
            <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
              ...
              <div class="space-y-6 py-6 px-5">
                ...
                <div>
                  <a href="#" class="flex w-full items-center justify-center rounded-md border border-transparent bg-cyan-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-cyan-700">Sign up</a>
                  <p class="mt-6 text-center text-base font-medium text-gray-500">
                    Existing customer?
                    {{ ' ' }}
                    <a href="#" class="text-cyan-600 hover:text-cyan-500">Sign in</a>
                  </p>
                </div>
              </div>
            </div>
      ...
      <div class="mx-auto max-w-7xl py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
        <div class="text-center">
          <h1 class="text-lg font-semibold text-cyan-600">Pricing</h1>
      ...
      <div class="relative mx-auto max-w-7xl lg:grid lg:grid-cols-2 lg:px-8">
        <div class="bg-white py-16 px-4 sm:py-24 sm:px-6 lg:px-0 lg:pr-8">
          <div class="mx-auto max-w-lg lg:mx-0">
            <h2 class="text-lg font-semibold text-cyan-600">Full-featured</h2>
            <p class="mt-2 text-2xl font-bold text-gray-900 sm:text-3xl sm:tracking-tight">Everything you need to talk with your customers</p>
            <dl class="mt-12 space-y-10">
              <div v-for="feature in features" :key="feature.name" class="relative">
                <dt>
                  <div class="absolute flex h-12 w-12 items-center justify-center rounded-md bg-gradient-to-r from-cyan-600 to-green-400">
        ...
        <div class="bg-gradient-to-r from-cyan-600 to-green-400 py-16 px-4 sm:py-24 sm:px-6 lg:flex lg:items-center lg:justify-end lg:bg-none lg:px-0 lg:pl-8">
          <div class="mx-auto w-full max-w-lg space-y-8 lg:mx-0">
            <div>
              ...
              <p class="relative grid grid-cols-2">
                ...
                <span class="pointer-events-none absolute flex h-12 w-full items-center justify-center" aria-hidden="true">
                  <PlusIcon class="h-6 w-6 text-cyan-200" aria-hidden="true" />
                </span>
                <span>
                  <span class="flex flex-col text-center">
                    <span class="text-5xl font-bold tracking-tight text-white">$4</span>
            ...
            <ul role="list" class="grid gap-px overflow-hidden rounded sm:grid-cols-2">
              <li v-for="item in checklist" :key="item" class="flex items-center bg-cyan-700 bg-opacity-50 py-4 px-4 text-base text-white">
                <CheckIcon class="h-6 w-6 text-cyan-200" aria-hidden="true" />
                <span class="ml-3">{{ item }}</span>
              </li>
            </ul>
            <a href="#" class="flex w-full items-center justify-center rounded-md border border-transparent bg-white py-4 px-8 text-lg font-medium leading-6 text-cyan-700 hover:bg-cyan-50 md:px-10">Get started today</a>
            <a href="#" class="block text-center text-base font-medium text-cyan-100 hover:text-white">Try our Lite plan for free</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-cyan-100 bg-opacity-25">
      <div class="mx-auto max-w-7xl py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
        <div class="lg:grid lg:grid-cols-2 lg:gap-8">
          <h2 class="mx-auto max-w-md text-center text-3xl font-bold tracking-tight text-cyan-900 lg:max-w-xl lg:text-left">The world's most innovative companies use our app</h2>
          <div class="mt-8 flow-root self-center lg:mt-0">
            <div class="-mt-4 -ml-8 flex flex-wrap justify-between lg:-ml-4">
              <div v-for="logo in logos" :key="logo.name" class="mt-4 ml-8 flex flex-shrink-0 flex-grow justify-center lg:ml-4 lg:flex-grow-0">
    ...
    <div class="mx-auto max-w-7xl py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
      ...
      <div class="mt-12">
        <dl class="space-y-10 md:grid md:grid-cols-2 md:grid-rows-2 md:gap-x-8 md:gap-y-12 md:space-y-0 lg:grid-cols-3">
          <div v-for="item in faqs" :key="item.id">
            <dt class="text-lg font-medium leading-6 text-gray-900">{{ item.question }}</dt>
    ...
    <div class="bg-gradient-to-r from-cyan-600 to-green-400">
      <div class="mx-auto max-w-2xl py-16 px-4 text-center sm:py-20 sm:px-6 lg:px-8">
        <h2 class="text-3xl font-bold tracking-tight text-white sm:text-4xl">
          ...
          <span class="block text-cyan-900">Start using our app today.</span>
        </h2>
        ...
        <a href="#" class="mt-8 inline-flex w-full items-center justify-center rounded-md border border-transparent bg-cyan-50 py-3 px-5 text-base font-medium text-cyan-700 hover:bg-cyan-100 sm:w-auto">Sign up for free</a>
      </div>
    </div>
      ...
      <div class="mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
        ...
        <div class="space-y-4 border-t border-gray-700 pt-8 lg:flex lg:items-center lg:justify-between lg:space-y-0 xl:mt-0">
          ...
          <form class="sm:flex sm:max-w-md">
            ...
            <input type="email" name="email-address" id="email-address" autocomplete="email" required="" class="w-full min-w-0 rounded-md border border-transparent bg-white py-2 px-4 placeholder-gray-500 focus:border-white focus:placeholder-gray-400 focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-900 sm:max-w-xs" placeholder="Enter your email" />
            <div class="mt-3 rounded-md sm:mt-0 sm:ml-3 sm:flex-shrink-0">
              <button type="submit" class="flex w-full items-center justify-center rounded-md border border-transparent bg-gray-700 py-2 px-4 text-base font-medium text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-900">Subscribe</button>
            </div>
          </form>
        </div>
