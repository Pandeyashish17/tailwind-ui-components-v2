  ...
  <div class="bg-white">
    ...
    <div class="relative">
          ...
          <div class="flex items-center justify-between border-b border-gray-200 py-6 md:justify-start md:space-x-10">
            <div class="flex justify-start lg:w-0 lg:flex-1">
              <a href="#">
                ...
                <img class="h-8 w-auto sm:h-10" src="https://tailwindui.com/img/logos/mark.svg?color=rose&shade=500" alt="" />
              </a>
            </div>
            ...
            <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
              <Popover class="relative" v-slot="{ open }">
                <PopoverButton :class="[open ? 'text-gray-900' : 'text-gray-500', 'group inline-flex items-center rounded-md bg-white text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2']">
                  <span>Solutions</span>
                  ...
                  <PopoverPanel class="absolute z-10 -ml-4 mt-3 w-screen max-w-md transform px-2 sm:px-0 lg:left-1/2 lg:ml-0 lg:-translate-x-1/2">
                    <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                      <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                        <a v-for="solution in header.solutions" :key="solution.name" :href="solution.href" class="-m-3 flex items-start rounded-lg p-3 hover:bg-gray-50">
                          <component :is="solution.icon" class="h-6 w-6 flex-shrink-0 text-rose-400" aria-hidden="true" />
                          <div class="ml-4">
                            <p class="text-base font-medium text-gray-900">{{ solution.name }}</p>
                      ...
                      <div class="space-y-6 bg-gray-50 px-5 py-5 sm:flex sm:space-y-0 sm:space-x-10 sm:px-8">
                        <div v-for="cta in header.callsToAction" :key="cta.name" class="flow-root">
                          <a :href="cta.href" class="-m-3 flex items-center rounded-md p-3 text-base font-medium text-gray-900 hover:bg-gray-100">
                            <component :is="cta.icon" class="h-6 w-6 flex-shrink-0 text-gray-400" aria-hidden="true" />
                            <span class="ml-3">{{ cta.name }}</span>
                          </a>
                        </div>
                      </div>
                    </div>
              ...
              <Popover class="relative" v-slot="{ open }">
                ...
                <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
                  <PopoverPanel class="absolute left-1/2 z-10 mt-3 w-screen max-w-md -translate-x-1/2 transform px-2 sm:px-0">
                    <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                      <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                        <a v-for="resource in header.resources" :key="resource.name" :href="resource.href" class="-m-3 flex items-start rounded-lg p-3 hover:bg-gray-50">
                          <component :is="resource.icon" class="h-6 w-6 flex-shrink-0 text-rose-400" aria-hidden="true" />
                          <div class="ml-4">
                      ...
                      <div class="bg-gray-50 px-5 py-5 sm:px-8 sm:py-8">
                        <div>
                          ...
                          <ul role="list" class="mt-4 space-y-4">
                            <li v-for="post in header.recentPosts" :key="post.name" class="truncate text-base">
            ...
            <div class="hidden items-center justify-end md:flex md:flex-1 lg:w-0">
              ...
              <a href="#" class="ml-8 inline-flex items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-rose-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-rose-600">Sign up</a>
            </div>
          </div>
        </div>
            ...
            <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
              <div class="px-5 pt-5 pb-6">
                <div class="flex items-center justify-between">
                  ...
                  <div class="-mr-2">
                    <PopoverButton class="inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-rose-500">
                ...
                <div class="mt-6">
                  <nav class="grid gap-y-8">
                    <a v-for="solution in header.solutions" :key="solution.name" :href="solution.href" class="-m-3 flex items-center rounded-md p-3 hover:bg-gray-50">
                      <component :is="solution.icon" class="h-6 w-6 flex-shrink-0 text-rose-400" aria-hidden="true" />
                      <span class="ml-3 text-base font-medium text-gray-900">{{ solution.name }}</span>
                    </a>
                  </nav>
                </div>
              </div>
              <div class="space-y-6 py-6 px-5">
                <div class="grid grid-cols-2 gap-y-4 gap-x-8">
                  ...
                  <a v-for="resource in header.resources" :key="resource.name" :href="resource.href" class="text-base font-medium text-gray-900 hover:text-gray-700">{{ resource.name }}</a>
                </div>
                <div>
                  <a href="#" class="flex w-full items-center justify-center rounded-md border border-transparent bg-rose-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-rose-600">Sign up</a>
                  <p class="mt-6 text-center text-base font-medium text-gray-500">
                    Existing customer?
                    {{ ' ' }}
                    <a href="#" class="text-rose-600 hover:text-rose-500">Sign in</a>
                  </p>
                </div>
              </div>
            </div>
      ...
      <div class="relative mx-auto max-w-2xl py-24 px-4 sm:px-6 lg:max-w-7xl lg:py-32 lg:px-8">
        <div class="relative">
          <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-5xl sm:leading-none lg:text-6xl">Pricing plans for teams of all sizes</h1>
    ...
    <main>
        ...
        <div class="mx-auto max-w-2xl space-y-12 px-4 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:space-y-0 lg:px-8">
          <div v-for="tier in pricing.tiers" :key="tier.title" class="relative flex flex-col rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
            <div class="flex-1">
              ...
              <p v-if="tier.mostPopular" class="absolute top-0 -translate-y-1/2 transform rounded-full bg-rose-500 py-1.5 px-4 text-sm font-semibold text-white">Most popular</p>
              ...
              <ul role="list" class="mt-6 space-y-6">
                <li v-for="feature in tier.features" :key="feature" class="flex">
                  <CheckIcon class="h-6 w-6 flex-shrink-0 text-rose-500" aria-hidden="true" />
                  <span class="ml-3 text-gray-500">{{ feature }}</span>
                </li>
              </ul>
            </div>

            <a href="#" :class="[tier.mostPopular ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-rose-50 text-rose-700 hover:bg-rose-100', 'mt-8 block w-full py-3 px-6 border border-transparent rounded-md text-center font-medium']">{{ tier.cta }}</a>
          </div>
        </div>
      </section>

      <!-- Logo Cloud -->
      <section aria-labelledby="customer-heading" class="mx-auto max-w-2xl py-24 px-4 sm:px-6 lg:max-w-7xl lg:py-32 lg:px-8">
      ...
      <div class="relative">
        ...
        <div aria-hidden="true" class="absolute inset-x-0 h-1/2 bg-gradient-to-b from-white to-gray-50" />
        ...
        <div class="xl:grid xl:grid-cols-3 xl:gap-8">
          <div class="space-y-8 xl:col-span-1">
            ...
            <div class="flex space-x-6">
              <a v-for="link in footer.social" :key="link.name" :href="link.href" class="text-gray-400 hover:text-gray-500">
          ...
          <div class="mt-12 grid grid-cols-2 gap-8 xl:col-span-2 xl:mt-0">
            <div class="md:grid md:grid-cols-2 md:gap-8">
              <div>
                ...
                <ul role="list" class="mt-4 space-y-4">
                  <li v-for="link in footer.solutions" :key="link.name">
                    <a :href="link.href" class="text-base text-gray-500 hover:text-gray-900">{{ link.name }}</a>
                  </li>
                </ul>
              </div>
              <div class="mt-12 md:mt-0">
                ...
                <ul role="list" class="mt-4 space-y-4">
                  <li v-for="link in footer.support" :key="link.name">
                    <a :href="link.href" class="text-base text-gray-500 hover:text-gray-900">{{ link.name }}</a>
                  </li>
                </ul>
              </div>
            ...
            <div class="md:grid md:grid-cols-2 md:gap-8">
              <div>
                ...
                <ul role="list" class="mt-4 space-y-4">
                  <li v-for="link in footer.company" :key="link.name">
                    <a :href="link.href" class="text-base text-gray-500 hover:text-gray-900">{{ link.name }}</a>
                  </li>
                </ul>
              </div>
              <div class="mt-12 md:mt-0">
                ...
                <ul role="list" class="mt-4 space-y-4">
                  <li v-for="link in footer.legal" :key="link.name">
                    <a :href="link.href" class="text-base text-gray-500 hover:text-gray-900">{{ link.name }}</a>
                  </li>
                </ul>
              </div>
