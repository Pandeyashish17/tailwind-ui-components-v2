  ...
  <div class="bg-white">
    <header class="relative bg-sky-800 pb-24 sm:pb-32">
      <div class="absolute inset-0">
        ...
        <div class="absolute inset-0 bg-gradient-to-l from-sky-800 to-cyan-700 mix-blend-multiply" aria-hidden="true" />
      </div>
        ...
        <nav class="relative mx-auto flex max-w-7xl items-center justify-between px-4 pt-6 pb-2 sm:px-6 lg:px-8" aria-label="Global">
          <div class="flex w-full items-center justify-between lg:w-auto">
            <a href="#">
              ...
              <img class="h-8 w-auto sm:h-10" src="https://tailwindui.com/img/logos/mark.svg?color=emerald&shade=300" alt="" />
            </a>
            <div class="-mr-2 flex items-center lg:hidden">
              <PopoverButton class="focus-ring-inset inline-flex items-center justify-center rounded-md bg-sky-800 bg-opacity-0 p-2 text-cyan-100 hover:bg-opacity-100 focus:outline-none focus:ring-2 focus:ring-white">
          ...
          <div class="hidden space-x-10 lg:ml-10 lg:flex">
            <a v-for="item in navigation" :key="item.name" :href="item.href" class="text-base font-medium text-white hover:text-cyan-100">{{ item.name }}</a>
          </div>
          <div class="hidden lg:flex lg:items-center lg:space-x-6">
            <a href="#" class="rounded-md border border-transparent bg-white bg-opacity-10 py-2 px-6 text-base font-medium text-white hover:bg-opacity-20">Login</a>
          </div>
        </nav>
          ...
          <PopoverPanel focus class="absolute inset-x-0 top-0 origin-top transform p-2 transition lg:hidden">
            <div class="overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
              <div class="flex items-center justify-between px-5 pt-4">
                ...
                <div class="-mr-2">
                  <PopoverButton class="inline-flex items-center justify-center rounded-md bg-white p-2 text-warm-gray-400 hover:bg-warm-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500">
              ...
              <div class="pt-5 pb-6">
                ...
                <div class="mt-6 px-5">
                  <a href="#" class="block w-full rounded-md border border-transparent bg-green-400 py-2 px-4 text-center font-medium text-white shadow hover:bg-green-500">Login</a>
                </div>
              </div>
            </div>
          </PopoverPanel>
      ...
      <div class="relative mx-auto mt-24 max-w-md px-4 sm:mt-32 sm:max-w-3xl sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <p class="mt-6 max-w-3xl text-xl text-cyan-100">Mattis amet hendrerit dolor, quisque lorem pharetra. Pellentesque lacus nisi urna, arcu sociis eu. Orci vel lectus nisl eget eget ut consectetur. Sit justo viverra non adipisicing elit distinctio.</p>
      </div>
    </header>
    ...
    <main>
        ...
        <div class="mx-auto max-w-md py-24 px-4 sm:max-w-3xl sm:py-32 sm:px-6 lg:max-w-7xl lg:px-8">
          <div class="divide-y divide-warm-gray-200">
            <section class="lg:grid lg:grid-cols-3 lg:gap-8" aria-labelledby="contact-heading">
              <h2 id="contact-heading" class="text-2xl font-bold text-warm-gray-900 sm:text-3xl sm:tracking-tight">Get in touch</h2>
              <div class="mt-8 grid grid-cols-1 gap-12 sm:grid-cols-2 sm:gap-x-8 sm:gap-y-12 lg:col-span-2 lg:mt-0">
                <div v-for="item in contactDetails" :key="item.name">
                  <h3 class="text-lg font-medium text-warm-gray-900">{{ item.name }}</h3>
            ...
            <section class="mt-16 pt-16 lg:grid lg:grid-cols-3 lg:gap-8" aria-labelledby="location-heading">
              ...
              <div class="mt-8 grid grid-cols-1 gap-12 sm:grid-cols-2 sm:gap-x-8 sm:gap-y-12 lg:col-span-2 lg:mt-0">
                <div v-for="location in locations" :key="location.city">
                  <h3 class="text-lg font-medium text-warm-gray-900">{{ location.city }}</h3>
                  <div class="mt-2 space-y-1 text-base text-warm-gray-500">
                    <p v-for="line in location.address" :key="line">{{ line }}</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <div class="mt-12 lg:col-span-2 lg:mt-0">
              <dl class="space-y-12">
                <div v-for="faq in faqs" :key="faq.id">
                  <dt class="text-lg font-medium text-warm-gray-900">{{ faq.question }}</dt>
                  <dd class="mt-2 text-base text-warm-gray-500">{{ faq.answer }}</dd>
                </div>
              </dl>
            </div>
          </div>
      ...
      <!-- Newsletter -->
      <div class="relative">
        <div class="absolute left-0 right-0 h-1/2 bg-warm-gray-50" aria-hidden="true" />
          ...
          <div class="rounded-3xl bg-gradient-to-l from-sky-800 to-cyan-700 py-10 px-6 sm:py-16 sm:px-12 lg:flex lg:items-center lg:py-20 lg:px-20">
            ...
            <div class="mt-12 sm:w-full sm:max-w-md lg:mt-0 lg:ml-8 lg:flex-1">
              <form class="sm:flex">
                ...
                <input id="email-address" name="email-address" type="email" autocomplete="email" required="" class="w-full rounded-md border-white px-5 py-3 placeholder-warm-gray-500 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-cyan-700" placeholder="Enter your email" />
      ...
      <div class="mx-auto max-w-md py-12 px-4 sm:max-w-3xl sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
        <div class="xl:grid xl:grid-cols-3 xl:gap-8">
          <div class="space-y-8 xl:col-span-1">
            <img class="h-10" src="https://tailwindui.com/img/logos/mark.svg?color=emerald&shade=400" alt="Company name" />
            <p class="text-base text-warm-gray-500">Making the world a better place through constructing elegant hierarchies.</p>
            <div class="flex space-x-6">
              <a v-for="item in footerNavigation.social" :key="item.name" :href="item.href" class="text-warm-gray-400 hover:text-warm-gray-500">
              ...
              <div>
                ...
                <ul role="list" class="mt-4 space-y-4">
                  <li v-for="item in footerNavigation.solutions" :key="item.name">
                    <a :href="item.href" class="text-base text-warm-gray-500 hover:text-warm-gray-900">{{ item.name }}</a>
                  </li>
                </ul>
              </div>
